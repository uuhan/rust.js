...
struct Socket;
impl Socket {
    extern "C" fn New(info: v8::FunctionCallbackInfo) {
        if info.IsConstructCall() {
            info.GetReturnValue().Set(info.This());
        }
    }
    extern "C" fn Connect(info: v8::FunctionCallbackInfo) {
        // TODO(Yorkie)
        info.GetReturnValue().Set(info.This());
    }
    extern "C" fn Disconnect(info: v8::FunctionCallbackInfo) {
        // TODO(Yorkie)
        info.GetReturnValue().Set(info.This());
    }
    extern "C" fn Init() -> v8::Function {
