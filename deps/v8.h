...
public:
    ...
    template <class S>
    V8_INLINE bool
    operator==(const Local<S>& that) const
    {
    ...
    template <class S>
    V8_INLINE bool
    operator==(const PersistentBase<S>& that) const
    {
        internal::Address* a = reinterpret_cast<internal::Address*>(this->val_);
        internal::Address* b = reinterpret_cast<internal::Address*>(that.val_);
        if (a == nullptr)
    ...
    template <class S>
    V8_INLINE bool
    operator!=(const Persistent<S>& that) const
    {
        return !operator==(that);
    }
     ...
     * In particular, this is only valid if the handle actually refers to a
...
public:
     ...
     */
    template <class S>
    V8_WARN_UNUSED_RESULT V8_INLINE bool
    ...
    template <class S>
    V8_INLINE Local<S>
    FromMaybe(Local<S> default_value) const
    {
...
public:
    ...
    // When first called, the embedder MUST Reset() the Global which triggered
    // the callback. The Global itself is unusable for anything else. No v8
    // other api calls may be called in the first callback. Should additional
    // work be required, the embedder must set a second pass callback, which
    // will be called after all the initial callbacks are processed. Calling
    // SetSecondPassCallback on the second pass will immediately crash.
    void
    SetSecondPassCallback(Callback callback) const
    {
        *callback_ = callback;
    }
...
public:
    ...
    /**
     * Turns this handle into a weak phantom handle without finalization
     * callback. The handle will be reset automatically when the garbage
     * collector detects that the object is no longer reachable. A related
     * function Isolate::NumberOfPhantomHandleResetsSinceLastCall returns how
     * many phantom handles were reset by the garbage collector.
     */
    ...
    /**
     * Annotates the strong handle with the given label, which is then used by
     * the heap snapshot generator as a name of the edge from the root to the
     * handle. The function does not take ownership of the label and assumes
     * that the label is valid as long as the handle is valid.
     */
    V8_INLINE void AnnotateStrongRetainer(const char* label);

    /**
     * Allows the embedder to tell the v8 garbage collector that a certain
     * object is alive. Only allowed when the embedder is asked to trace its
     * heap by EmbedderHeapTracer.
     */
    V8_DEPRECATE_SOON(
...
public:
    ...
    V8_INLINE static void
    Copy(const Persistent<S, M>& source, NonCopyablePersistent* dest)
    {
        Uncompilable<Object>();
    }
...
struct CopyablePersistentTraits {
    ...
    Copy(const Persistent<S, M>& source, CopyablePersistent* dest)
    {
...
public:
    ...
    operator=(const Persistent& that)
    {
        Copy(that);
        return *this;
    }
    template <class S, class M2>
    V8_INLINE Persistent&
    operator=(const Persistent<S, M2>& that)
    {  // NOLINT
        Copy(that);
        return *this;
    }
    ...
    template <class S>
    V8_INLINE static Persistent<T>&
    Cast(const Persistent<S>& that)
    {  // NOLINT
#ifdef V8_ENABLE_CHECKS
        // If we're going to perform the type check then we have to check
        // that the handle isn't empty before doing the checked cast.
...
public:
    ...
    /**
     * Returns true if this TracedGlobal is empty, i.e., has not been assigned
     * an object.
     */
    bool
    IsEmpty() const
    ...
    template <class S>
    V8_INLINE bool
    operator==(const TracedGlobal<S>& that) const
    {
        internal::Address* a = reinterpret_cast<internal::Address*>(this->val_);
        internal::Address* b = reinterpret_cast<internal::Address*>(that.val_);
        if (a == nullptr)
...
private:
    ...
    // with a HeapObject in their shortcuts.
    friend class Object;
    friend class Context;
};
...
public:
    ...
    template <class T>
    V8_INLINE MaybeLocal<T>
    EscapeMaybe(MaybeLocal<T> value)
    {
...
public:
    V8_INLINE
    ScriptOriginOptions(bool is_shared_cross_origin = false,
...
public:
    ...
    /**
     * For a module in kErrored status, this returns the corresponding
     * exception.
     */
...
public:
     ...
     * future compilations. The data is produced if the CompilerOptions passed
     * to the compilation functions in ScriptCompiler contains
     * produce_data_to_cache = true. The data to cache can then can be retrieved
     * from UnboundScript.
     */
    struct V8_EXPORT CachedData {
        ...
        // is destroyed. If the policy is BufferOwned, the given data will be
        // deleted (with delete[]) when the CachedData object is destroyed.
        CachedData(const uint8_t* data, int length,
                   BufferPolicy buffer_policy = BufferNotOwned);
        ~CachedData();
        // TODO(marja): Async compilation; add constructors which take a
        // callback which will be called when V8 no longer needs the data.
        const uint8_t* data;
        int length;
        bool rejected;
    ...
    public:
        ...
        // the caller. The CachedData object is alive as long as the Source
        // object is alive.
    ...
    /**
     * For streaming incomplete script data to V8. The embedder should implement
    ...
    public:
         ...
         * This function will be called on a background thread, so it's OK to
         * block and wait for the data, if the embedder doesn't have data yet.
         * Returns the length of the data returned. When the data ends,
         * GetMoreData should return 0. Caller takes ownership of the data.
         *
         * When streaming UTF-8 data, V8 handles multi-byte characters split
         * between two data chunks, but doesn't handle multi-byte characters
         * split between more than two data chunks. The embedder can avoid this
         * problem by always returning at least 2 bytes of data.
         *
         * When streaming UTF-16 data, V8 does not handle characters split
         * between two data chunks. The embedder has to make sure that chunks
         * have an even length.
         *
         * If the embedder wants to cancel the streaming, they should make the
         * next GetMoreData call return 0. V8 will interpret it as end of data
         * (and most probably, parsing will fail). The streaming task will
         * return as soon as V8 has parsed the data it received so far.
         */
     ...
     * must be kept alive while the streaming task is run (see
     ...
     * \return Compiled script object (context independent; for running it must
     ...
     *
     * This API allows to start the streaming with as little data as possible,
     * and the remaining data (for example, the ScriptOrigin) is passed to
     * Compile.
     */
     ...
     * (ScriptStreamingTask has been run). V8 doesn't construct the source
     * string during streaming, so the embedder needs to pass the full source
     * here.
     */
    static V8_WARN_UNUSED_RESULT MaybeLocal<Script> Compile(
...
private:
    static V8_WARN_UNUSED_RESULT MaybeLocal<UnboundScript>
    CompileUnboundInternal(Isolate* isolate, Source* source,
...
public:
     ...
     */
    V8_WARN_UNUSED_RESULT Maybe<int> GetLineNumber(
        Local<Context> context) const;
    ...
    V8_WARN_UNUSED_RESULT Maybe<int> GetStartColumn(
        Local<Context> context) const;
...
public:
    ...
    enum StackTraceOptions {
        ...
        kDetailed =
            kOverview | kIsEval | kIsConstructor | kScriptNameOrSourceURL
    };
    ...
    static Local<StackTrace> CurrentStackTrace(
        Isolate* isolate, int frame_limit,
...
public:
     ...
     * call.
     * This method will return Message::kNoColumnInfo if it is unable to
...
public:
    ...
    public:
        ...
        /**
         * Handles the case where a DataCloneError would be thrown in the
         * structured clone spec. Other V8 embedders may throw some other
         * appropriate exception type.
         */
        virtual void ThrowDataCloneError(Local<String> message) = 0;

        /**
         * The embedder overrides this method to write some kind of host object,
         * if possible. If not, a suitable exception should be thrown and
         ...
         * serialized in this buffer. When deserializing, this ID will be passed
         * to ValueDeserializer::GetSharedArrayBufferFromId as |clone_id|.
         *
        ...
        /**
         * Allocates memory for the buffer of at least the size provided. The
         * actual size (which may be greater or equal) is written to
         * |actual_size|. If no buffer has been allocated yet, nullptr will be
         * provided.
         *
         * If the memory cannot be allocated, nullptr should be returned.
         * |actual_size| will be ignored. It is assumed that |old_buffer| is
         * still valid in this case and has not been modified.
         *
     ...
     * once the buffer is released. The contents are undefined if a previous
     * write has failed. Ownership of the buffer is transferred to the caller.
     */
...
public:
    ...
    public:
        ...
        /**
         * The embedder overrides this method to read some kind of host object,
         * if possible. If not, a suitable exception should be thrown and
     ...
     *
     * Don't use this unless you need to read data written by previous versions
     * of blink::ScriptValueSerializer.
     */
...
public:
    ...
    /**
     * Returns true if this value is an array. Note that it will return false
     * for an Proxy for an array.
     */
    bool IsArray() const;
    ...
    V8_WARN_UNUSED_RESULT MaybeLocal<Uint32> ToUint32(
        Local<Context> context) const;
    V8_WARN_UNUSED_RESULT MaybeLocal<Int32> ToInt32(
        Local<Context> context) const;
    ...
    V8_DEPRECATED("BooleanValue can never throw. Use Isolate version.",
                  V8_WARN_UNUSED_RESULT Maybe<bool> BooleanValue(
                      Local<Context> context) const);
    V8_WARN_UNUSED_RESULT Maybe<double> NumberValue(
        Local<Context> context) const;
    V8_WARN_UNUSED_RESULT Maybe<int64_t> IntegerValue(
        Local<Context> context) const;
    V8_WARN_UNUSED_RESULT Maybe<uint32_t> Uint32Value(
        Local<Context> context) const;
    V8_WARN_UNUSED_RESULT Maybe<int32_t> Int32Value(
        Local<Context> context) const;
...
public:
    ...
    enum WriteOptions {
        ...
        // unicode replacement character. Needs to be set to guarantee valid
        // UTF-8 output.
        REPLACE_INVALID_UTF8 = 8
    };

    // 16-bit character codes.
    int Write(Isolate* isolate, uint16_t* buffer, int start = 0,
    ...
    protected:
         ...
         * |ExternalStringResource::data()| has to be stable between |Lock()|
         * and |Unlock()|, that is the string must behave as is |IsCacheable()|
         * returned true.
         *
         * These two functions must be thread-safe, and can be called from
         * anywhere. They also must handle lock depth, in the sense that each
         * can be called several times, from different threads, and unlocking
         * should only happen when the balance of Lock() and Unlock() calls is
         * 0.
         */
        virtual void
        Lock() const
     ...
     * ExternalStringResourceBase regardless of the encoding, otherwise return
     * NULL.  The encoding of the string is returned in encoding_out.
     */
    ...
    /** Allocates a new string from UTF-16 data. Only returns an empty value
     * when length > kMaxLength. **/
     ...
     * the destructor of the external string resource.
     */
     ...
     * heap will use the external string resource. The external string
     * resource's character contents need to be equivalent to this string.
    ...
    /**
     * Creates a new external string using the one-byte data defined in the
     * given resource. When the external string is no longer live on V8's heap
     * the resource will be disposed by calling its Dispose method. The caller
     * of this function should not otherwise delete or modify the resource.
     * Neither should the underlying buffer be deallocated or modified except
     * through the destructor of the external string resource.
     */
    static V8_DEPRECATE_SOON(
        "Use maybe version",
     ...
     * toString() method of the object) then the length() method returns 0 and
...
public:
    ...
    /**
     * Create a symbol. If name is not empty, it will be used as the
     * description.
     */
...
public:
    ...
    /**
     * Create a private symbol. If name is not empty, it will be the
     * description.
     */
...
public:
    ...
    //
    // Defines a configurable, writable, enumerable property with the given
    // value on the object unless the property already exists and is not
    // configurable or the object is not extensible.
    //
     ...
     * true, if the object has the property, either own or on the prototype
     * chain. Interceptors, i.e., PropertyQueryCallbacks, are called if present.
     *
    ...
    /**
     * Attempts to create a property with the given name which behaves like a
     * data property, except that the provided getter is invoked (and provided
     * with the data value) to supply its value the first time it is read. After
     * the property is accessed once, it is replaced with an ordinary data
     * property.
     *
    ...
    V8_INLINE static int
    InternalFieldCount(const TracedGlobal<Object>& object)
    {
        return object.val_->InternalFieldCount();
    }
    ...
    /** Same as above, but works for TracedGlobal. */
    V8_INLINE static void*
    GetAlignedPointerFromInternalField(const TracedGlobal<Object>& object,
                                       int index)
    {
        return object.val_->GetAlignedPointerFromInternalField(index);
    }
     ...
     * such a field, GetAlignedPointerFromInternalField must be used, everything
     ...
     * property without causing side effects, i.e., without calling
     * interceptors.
     *
     ...
     */
    V8_WARN_UNUSED_RESULT Maybe<bool> HasRealNamedProperty(
        Local<Context> context, Local<Name> key);
    V8_DEPRECATED("Use maybe version",
     ...
     */
    V8_WARN_UNUSED_RESULT MaybeLocal<Value>
    GetRealNamedPropertyInPrototypeChain(Local<Context> context,
                                         Local<Name> key);

    /**
     * Gets the property attributes of a real property in the prototype chain,
     * which can be None or any combination of ReadOnly, DontEnum and
     * DontDelete. Interceptors in the prototype chain are not called.
     */
    ...
    V8_INLINE static Local<Context>
    CreationContext(const PersistentBase<Object>& object)
    {
     ...
     * ObjectTemplate::SetCallAsFunctionHandler method.
     */
    V8_WARN_UNUSED_RESULT MaybeLocal<Value> CallAsFunction(
        Local<Context> context, Local<Value> recv, int argc,
        Local<Value> argv[]);
...
private:
    ...
    friend class PersistentValueMapBase;
    V8_INLINE void
    SetInternal(internal::Address value)
    {
        *value_ = value;
    }
...
protected:
    ...
    V8_INLINE
    PropertyCallbackInfo(internal::Address* args)
        : args_(args)
    {
    }
...
public:
    ...
    V8_WARN_UNUSED_RESULT MaybeLocal<Object>
     ...
     * function. Used to facilitate debugging and profiling of JavaScript code.
     */
...
public:
    /**
     * State of the promise. Each value corresponds to one of the possible
     * values of the [[PromiseState]] field.
     */
    ...
    public:
        /**
         * Create a new resolver, along with an associated promise in pending
         * state.
         */
...
public:
     ...
     * TODO(wasm): Merge this with CompiledWasmModule once code sharing is
     * always enabled.
     */
    ...
    /**
     * Get the compiled module for this module object. The compiled module can
     * be shared by several module objects.
     */
    CompiledWasmModule GetCompiledModule();

    /**
     * If possible, deserialize the module, otherwise compile it from the
     * provided uncompiled bytes.
     */
...
private:
    ...
    static MaybeLocal<WasmModuleObject> Compile(Isolate* isolate,
                                                const uint8_t* start,
                                                size_t length);
    static MemorySpan<const uint8_t>
    AsReference(const OwnedBuffer& buff)
    {
...
public:
    ...
    /**
     * Unpacks a {WasmStreaming} object wrapped in a  {Managed} for the
     * embedder. Since the embedder is on the other side of the API, it cannot
     * unpack the {Managed} itself.
     */
...
public:
     ...
     *
     * Memory allocated through this allocator by V8 is accounted for as
     * external memory by V8. Note that V8 keeps track of the memory for all
     * internalized |ArrayBuffer|s. Responsibility for tracking external memory
     * (using Isolate::AdjustAmountOfExternalAllocatedMemory) is handed over to
     * the embedder upon externalization and taken over upon internalization
     * (creating an internalized buffer from an existing buffer).
     *
    ...
    public:
        ...
        /**
         * ArrayBuffer allocation mode. kNormal is a malloc/free style
         * allocation, while kReservation is for larger allocations with the
         ...
         *
         * Caller takes ownership, i.e. the returned object needs to be freed
         * using |delete allocator| once it is no longer in use.
         */
     ...
     * and byte length.
     *
     * The Data pointer of ArrayBuffer::Contents must be freed using the
     * provided deleter, which will call ArrayBuffer::Allocator::Free if the
     * buffer was allocated with ArraryBuffer::Allocator::Allocate.
     */
    class V8_EXPORT Contents
    {  // NOLINT
    public:
        using DeleterCallback = void (*)(void* buffer, size_t length,
                                         void* info);
     ...
     * Detaching sets the byte length of the buffer and all typed arrays to
     * zero, preventing JavaScript from ever accessing underlying backing store.
...
public:
    static Local<BigInt64Array> New(Local<ArrayBuffer> array_buffer,
                                    size_t byte_offset, size_t length);
    static Local<BigInt64Array> New(
        Local<SharedArrayBuffer> shared_array_buffer, size_t byte_offset,
        size_t length);
...
public:
    static Local<BigUint64Array> New(Local<ArrayBuffer> array_buffer,
                                     size_t byte_offset, size_t length);
    static Local<BigUint64Array> New(
        Local<SharedArrayBuffer> shared_array_buffer, size_t byte_offset,
        size_t length);
...
public:
    ...
    /**
     * Create a new SharedArrayBuffer over an existing memory block.  The
     * created array buffer is immediately in externalized state unless
     * otherwise specified. The memory block will not be reclaimed when a
     * created SharedArrayBuffer is garbage-collected.
     */
...
public:
     ...
     *
     * The InstanceTemplate of this FunctionTemplate must have access checks
     * with handlers installed.
     */
     ...
     * property is used for this template. This is mutually exclusive with
     *setting a prototype template indirectly by calling PrototypeTemplate() or
     *using Inherit().
     **/
    void SetPrototypeProviderTemplate(
     ...
     * function call.  Currently defaults to true, but this is subject to
     * change.
     */
...
enum class PropertyHandlerFlags {
    ...
    /** Will not call into interceptor for properties on the receiver or
     * prototype chain, i.e., only call into interceptor for properties that do
     * not exist. Currently only valid for named interceptors.
     */
    kNonMasking = 1 << 1,
...
public:
     ...
     * object.
     *
     * @param configuration The NamedPropertyHandlerConfiguration that defines
    ...
    /**
     * Makes the ObjectTemplate for an immutable prototype exotic object, with
     * an immutable __proto__.
     */
...
public:
    ...
    virtual Local<FunctionTemplate>
    GetNativeFunctionTemplate(Isolate* isolate, Local<String> name)
    {
...
public:
    ...
    size_t
    max_semi_space_size_in_kb() const
    {
    ...
    void
    set_max_semi_space_size_in_kb(size_t limit_in_kb)
    {
        max_semi_space_size_in_kb_ = limit_in_kb;
    }

    size_t
    max_old_space_size() const
    {
        return max_old_space_size_;
    }
    void
    set_max_old_space_size(size_t limit_in_mb)
    {
    ...
    void
    set_stack_limit(uint32_t* value)
    {
        stack_limit_ = value;
    }
    size_t
    code_range_size() const
    {
        return code_range_size_;
    }
    void
    set_code_range_size(size_t limit_in_mb)
    {
        code_range_size_ = limit_in_mb;
    }
    size_t
    max_zone_pool_size() const
    {
        return max_zone_pool_size_;
    }
    void
    set_max_zone_pool_size(size_t bytes)
    {
...
public:
     ...
     * Will try to reconstruct the original stack trace from the exception
     * value, or capture the current stack trace if not available.
     */
...
public:
    ...
    V8_INLINE Local<Promise>
    GetPromise() const
    {
        return promise_;
    }
    V8_INLINE PromiseRejectEvent
    GetEvent() const
    {
        return event_;
    }
...
struct JitCodeEvent {
    ...
    // There are two different kinds of JitCodeEvents, one for JIT code
    // generated by the optimizing compiler, and one for byte code generated for
    // the interpreter.  For JIT_CODE events, the |code_start| member of the
    // event points to the beginning of jitted assembly code, while for
    // BYTE_CODE events, |code_start| points to the first bytecode of the
...
enum RAILMode {
    ...
    // Idle performance mode: The embedder is idle. V8 can complete deferred
    // work
    // in this mode.
    PERFORMANCE_IDLE,
...
public:
    ...
    virtual void
    VisitExternalString(Local<String> string)
    {
    }
};
...
public:
    ...
    virtual void
    VisitPersistentHandle(Persistent<Value>* value, uint16_t class_id)
    {
    }
};
...
public:
    ...
    public:
        ...
        virtual void VisitTracedGlobalHandle(
            const TracedGlobal<Value>& value) = 0;
    };

    virtual ~EmbedderHeapTracer() = default;

    /**
     * Iterates all TracedGlobal handles created for the v8::Isolate the tracer
     * is attached to.
     */
     ...
     *
     * The embedder is expected to trace its heap starting from wrappers
     * reported by RegisterV8References method, and report back all reachable
     * wrappers. Furthermore, the embedder is expected to stop tracing by the
     * given deadline. A deadline of infinity means that tracing should be
     * finished.
     *
    ...
    /*
     * Returns true if there no more tracing work to be done (see
     * AdvanceTracing) and false otherwise.
     */
     ...
     */
    virtual bool
    IsRootForNonTracingGC(const v8::TracedGlobal<v8::Value>& handle)
    {
        return true;
    }
...
public:
    ...
    struct CreateParams {
         ...
         * is notified each time code is added, moved or removed.
         */
        ...
        /**
         * Explicitly specify a startup snapshot blob. The embedder owns the
         * blob.
         */
        ...
        /**
         * The ArrayBuffer::Allocator to use for allocating and freeing the
         * backing store of ArrayBuffers.
         */
        ArrayBuffer::Allocator* array_buffer_allocator;

        /**
         * Specifies an optional nullptr-terminated array of raw addresses in
         * the embedder that V8 can match against during serialization and use
         * for deserialization. This array and its content must stay valid for
         * the entire lifetime of the isolate.
         */
        const intptr_t* external_references;

        /**
         * Whether calling Atomics.wait (a function that may block) is allowed
         * in this isolate. This can also be configured via SetAllowAtomicsWait.
         */
        bool allow_atomics_wait;

        /**
         * Termination is postponed when there is no active
    ...
    enum UseCounterFeature {
        ...
        // web_feature.mojom, UseCounterCallback.cpp, and enums.xml. V8 changes
        // to this list need to be landed first, then changes on the Chromium
        // side.
     ...
     *   statistics of objects of given type, which were live in the previous
     * GC. \param type_index The index of the type of object to fill details
     * about, which ranges from 0 to NumberOfTrackedHeapObjectTypes() - 1.
     * \returns true on success.
     */
    bool GetHeapObjectStatisticsAtLastGC(
     ...
     * \note GetStackSample should only be called when the JS thread is paused
     * or interrupted. Otherwise the behavior is undefined.
     */
     ...
     * global garbage collections. Registering externally allocated memory will
     * trigger global garbage collections more often than it would otherwise in
     * an attempt to garbage collect the JavaScript objects that keep the
     * externally allocated memory alive.
     *
     ...
     * an exception has been scheduled it is illegal to invoke any JavaScript
    ...
    public:
         ...
         * with |kAPIStopped|.
         *
         * This function may be called from another thread. The caller has to
         * ensure through proper synchronization that it is not called after the
         * finishing |AtomicsWaitCallback|.
         *
         * Note that the ECMAScript specification does not plan for the
         * possibility of wakeups that are neither coming from a timeout or an
         * `Atomics.wake()` call, so this may invalidate assumptions made by
         * existing code. The embedder may accordingly wish to schedule an
         * exception in the finishing |AtomicsWaitCallback|.
         */
        void Wake();
    };
    ...
    /**
     * Optional notification to tell V8 the current isolate is used for
     * debugging and requires higher heap limit.
     */
     ...
     * \note the set of events declared in JitCodeEvent::EventType is expected
     * to grow over time, and the JitCodeEvent structure is expected to accrue
     ...
     * V8's 'builtins' will not be in this range if embedded builtins is
     * enabled. Instead, see GetEmbeddedCodeRange.
     *
    ...
    /**
     * Add a callback to invoke in case the heap size is close to the heap
     * limit. If multiple callbacks are added, only the most recently added
     * callback is invoked.
     */
    ...
    V8_DEPRECATED("The callback set in SetWasmStreamingCallback is used now",
                  void SetWasmCompileStreamingCallback(
                      ApiImplementationCallback callback));
     ...
     * called. Otherwise, the exception object will be passed to the callback
     * instead.
     */
     ...
     * dependent handles. This will visit handles to young objects created since
     * the last garbage collection but is free to visit an arbitrary superset of
     * these objects.
     */
    V8_DEPRECATED("Use VisitHandlesWithClassIds",
     ...
     * in this isolate. This can also be configured via
...
public:
    ...
    static bool InitializeICUDefaultLocation(
        const char* exec_path, const char* icu_data_file = nullptr);
     ...
     *
     * This function determines whether a memory access violation can be
     * recovered by V8. If so, it will return true and modify context to return
     * to a code fragment that can recover from the fault. Otherwise,
     * TryHandleSignal will return false.
     *
     ...
     * which points to a ucontext_t structure.
     */
...
private:
    ...
    static internal::Address* GlobalizeTracedReference(
...
public:
     ...
     * global object template to create one, to be provided upon
     * deserialization.
     *
     ...
     * via Context::GetDataFromSnapshot after deserialization. This data does
     ...
     * via Isolate::GetDataFromSnapshot after deserialization. This data does
     * not survive when a new snapshot is created from an existing snapshot.
     * \returns the index for retrieval.
     */
    template <class T>
...
public:
    ...
    /**
     * Short-hand for ToChecked(), which doesn't return a value. To be used,
...
public:
    ...
    /**
     * Returns the exception caught by this try/catch block.  If no exception
     * has been caught an empty handle is returned.
     *
     ...
     */
    static void*
    JSStackComparableAddress(TryCatch* handler)
    {
        if (handler == nullptr)
            return nullptr;
...
public:
     ...
     * done that way for security reasons (for more details see
     ...
     * the global proxy will fail. Instead, the access check handlers are
     * invoked.
     *
    ...
    public:
        /**
         * |backup_incumbent_context| is pushed onto the backup incumbent
        ...
        /**
         * Returns address that is comparable with JS stack address.  Note that
         * JS stack may be allocated separately from the native stack.  See also
         * |TryCatch::JSStackComparableAddress| for details.
         */
        uintptr_t
        JSStackComparableAddress() const
        {
...
public:
    /**
     * Attempt to unwind the stack to the most recent C++ frame. This function
     * is signal-safe and does not access any V8 state and thus doesn't require
     * an Isolate.
     *
     * The unwinder needs to know the location of the JS Entry Stub (a piece of
     * code that is run when C++ code calls into generated JS code). This is
     * used for edge cases where the current frame is being constructed or torn
     * down when the stack sample occurs.
     *
     * The unwinder also needs the virtual memory range of all possible V8 code
     * objects. There are two ranges required - the heap code range and the
     * range for code embedded in the binary. The V8 API provides all required
     * inputs via an UnwindState object through the Isolate::GetUnwindState()
     * API. These values will not change after Isolate initialization, so the
     * same |unwind_state| can be used for multiple calls.
     *
     * \param unwind_state Input state for the Isolate that the stack comes
     * from. \param register_state The current registers. This is an in-out
     * param that will be overwritten with the register values after unwinding,
     * on success. \param stack_base The resulting stack pointer and frame
     * pointer values are bounds-checked against the stack_base and the original
     * stack pointer value to ensure that they are valid locations in the given
     * stack. If these values or any intermediate frame pointer values used
     * during unwinding are ever out of these bounds, unwinding will fail.
     *
...
template <class T>
Local<T>
Local<T>::New(Isolate* isolate, const PersistentBase<T>& that)
{
    return New(isolate, that.val_);
}
...
template <class T>
Local<T>
Local<T>::New(Isolate* isolate, const TracedGlobal<T>& that)
{
    return New(isolate, that.val_);
}
...
template <class S>
void
Eternal<T>::Set(Isolate* isolate, Local<S> handle)
{
    TYPE_CHECK(T, S);
...
template <class T>
Local<T>
Eternal<T>::Get(Isolate* isolate) const
{
...
template <class T>
void*
WeakCallbackInfo<T>::GetInternalField(int index) const
{
#ifdef V8_ENABLE_CHECKS
...
template <class T>
T*
PersistentBase<T>::New(Isolate* isolate, T* that)
{
    if (that == nullptr)
        return nullptr;
...
template <class S, class M2>
void
Persistent<T, M>::Copy(const Persistent<S, M2>& that)
{
    TYPE_CHECK(T, S);
    this->Reset();
    if (that.IsEmpty())
...
template <class T>
bool
PersistentBase<T>::IsIndependent() const
{
...
template <class T>
bool
PersistentBase<T>::IsNearDeath() const
{
    typedef internal::Internals I;
    if (this->IsEmpty())
        return false;
...
template <class S>
void
PersistentBase<T>::Reset(Isolate* isolate, const Local<S>& other)
{
    TYPE_CHECK(T, S);
    Reset();
    if (other.IsEmpty())
...
template <class S>
void
PersistentBase<T>::Reset(Isolate* isolate, const PersistentBase<S>& other)
{
    TYPE_CHECK(T, S);
    Reset();
    if (other.IsEmpty())
...
template <typename P>
V8_INLINE void
PersistentBase<T>::SetWeak(P* parameter,
                           typename WeakCallbackInfo<P>::Callback callback,
                           WeakCallbackType type)
{
...
template <class T>
void
PersistentBase<T>::AnnotateStrongRetainer(const char* label)
{
...
template <class T>
void
PersistentBase<T>::RegisterExternalReference(Isolate* isolate) const
{
    if (IsEmpty())
        return;
...
template <class T>
void
PersistentBase<T>::MarkIndependent()
{
    typedef internal::Internals I;
    if (this->IsEmpty())
        return;
...
template <class T>
void
PersistentBase<T>::SetWrapperClassId(uint16_t class_id)
{
    typedef internal::Internals I;
    if (this->IsEmpty())
        return;
...
template <class T>
uint16_t
PersistentBase<T>::WrapperClassId() const
{
    typedef internal::Internals I;
    if (this->IsEmpty())
        return 0;
...
template <class T>
T*
TracedGlobal<T>::New(Isolate* isolate, T* that, T** slot)
{
    if (that == nullptr)
        return nullptr;
    internal::Address* p = reinterpret_cast<internal::Address*>(that);
...
template <class S>
void
TracedGlobal<T>::Reset(Isolate* isolate, const Local<S>& other)
{
    TYPE_CHECK(T, S);
    Reset();
    if (other.IsEmpty())
...
template <class T>
void
TracedGlobal<T>::SetWrapperClassId(uint16_t class_id)
{
    typedef internal::Internals I;
    if (IsEmpty())
        return;
...
template <class T>
uint16_t
TracedGlobal<T>::WrapperClassId() const
{
    typedef internal::Internals I;
    if (IsEmpty())
        return 0;
...
template <class T>
void
TracedGlobal<T>::SetFinalizationCallback(
...
template <typename S>
void
ReturnValue<T>::Set(const Persistent<S>& handle)
{
    TYPE_CHECK(T, S);
...
template <typename S>
void
ReturnValue<T>::Set(const Global<S>& handle)
{
    TYPE_CHECK(T, S);
    if (V8_UNLIKELY(handle.IsEmpty())) {
        *value_ = GetDefaultValue();
...
template <typename S>
void
ReturnValue<T>::Set(const TracedGlobal<S>& handle)
{
    TYPE_CHECK(T, S);
    if (V8_UNLIKELY(handle.IsEmpty())) {
        *value_ = GetDefaultValue();
...
template <typename S>
void
ReturnValue<T>::Set(const Local<S> handle)
{
    TYPE_CHECK(T, S);
    if (V8_UNLIKELY(handle.IsEmpty())) {
        *value_ = GetDefaultValue();
...
template <typename T>
Isolate*
ReturnValue<T>::GetIsolate() const
{
...
template <typename T>
Local<Object>
FunctionCallbackInfo<T>::This() const
{
...
template <typename T>
Local<Object>
FunctionCallbackInfo<T>::Holder() const
{
    return Local<Object>(
...
template <typename T>
Local<Value>
FunctionCallbackInfo<T>::NewTarget() const
{
    return Local<Value>(
...
template <typename T>
Local<Value>
FunctionCallbackInfo<T>::Data() const
{
...
template <typename T>
Isolate*
FunctionCallbackInfo<T>::GetIsolate() const
{
...
template <typename T>
ReturnValue<T>
FunctionCallbackInfo<T>::GetReturnValue() const
{
...
template <typename T>
bool
FunctionCallbackInfo<T>::IsConstructCall() const
{
...
template <typename T>
int
FunctionCallbackInfo<T>::Length() const
{
    return length_;
}
...
Local<Value>
ScriptOrigin::ResourceName() const
{
    return resource_name_;
}

Local<PrimitiveArray>
ScriptOrigin::HostDefinedOptions() const
{
    return host_defined_options_;
}

Local<Integer>
ScriptOrigin::ResourceLineOffset() const
{
...
Local<Integer>
ScriptOrigin::ResourceColumnOffset() const
{
...
Local<Value>
ScriptOrigin::SourceMapUrl() const
{
...
const ScriptOriginOptions&
ScriptCompiler::Source::GetResourceOptions() const
{
...
void
Template::Set(Isolate* isolate, const char* name, Local<Data> value)
{
...
AccessorSignature*
AccessorSignature::Cast(Data* data)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(data);
#endif
...
Object::GetInternalField(int index)
{
    ...
    // we know where to find the internal fields and can return the value
    // directly.
...
void*
Object::GetAlignedPointerFromInternalField(int index)
{
#if !defined(V8_ENABLE_CHECKS) && !defined(V8_COMPRESS_POINTERS)
    typedef internal::Address A;
    typedef internal::Internals I;
...
String::GetExternalStringResourceBase(String::Encoding* encoding_out) const
{
    typedef internal::Address A;
    typedef internal::Internals I;
    A obj = *reinterpret_cast<const A*>(this);
...
bool
Value::QuickIsNullOrUndefined() const
{
    typedef internal::Address A;
    typedef internal::Internals I;
    A obj = *reinterpret_cast<const A*>(this);
...
StringObject*
StringObject::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
SymbolObject*
SymbolObject::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
NumberObject*
NumberObject::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
BigIntObject*
BigIntObject::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
BooleanObject*
BooleanObject::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
WasmModuleObject*
WasmModuleObject::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Promise::Resolver*
Promise::Resolver::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
ArrayBufferView*
ArrayBufferView::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
TypedArray*
TypedArray::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Uint8Array*
Uint8Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Uint16Array*
Uint16Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Int16Array*
Int16Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Uint32Array*
Uint32Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Int32Array*
Int32Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Float32Array*
Float32Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Float64Array*
Float64Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
BigInt64Array*
BigInt64Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
BigUint64Array*
BigUint64Array::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
Uint8ClampedArray*
Uint8ClampedArray::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
SharedArrayBuffer*
SharedArrayBuffer::Cast(v8::Value* value)
{
#ifdef V8_ENABLE_CHECKS
    CheckCast(value);
#endif
...
template <typename T>
Isolate*
PropertyCallbackInfo<T>::GetIsolate() const
{
...
template <typename T>
Local<Value>
PropertyCallbackInfo<T>::Data() const
{
...
template <typename T>
Local<Object>
PropertyCallbackInfo<T>::This() const
{
...
template <typename T>
Local<Object>
PropertyCallbackInfo<T>::Holder() const
{
...
template <typename T>
ReturnValue<T>
PropertyCallbackInfo<T>::GetReturnValue() const
{
...
template <typename T>
bool
PropertyCallbackInfo<T>::ShouldThrowOnError() const
{
    typedef internal::Internals I;
...
void
Isolate::SetData(uint32_t slot, void* data)
{
    typedef internal::Internals I;
...
template <class T>
MaybeLocal<T>
Isolate::GetDataFromSnapshotOnce(size_t index)
{
...
Local<Value>
Context::GetEmbedderData(int index)
{
#if !defined(V8_ENABLE_CHECKS) && !defined(V8_COMPRESS_POINTERS)
    typedef internal::Address A;
    typedef internal::Internals I;
...
void*
Context::GetAlignedPointerFromEmbedderData(int index)
{
#if !defined(V8_ENABLE_CHECKS) && !defined(V8_COMPRESS_POINTERS)
    typedef internal::Internals I;
...
template <class T>
MaybeLocal<T>
Context::GetDataFromSnapshotOnce(size_t index)
{
    T* data = reinterpret_cast<T*>(GetDataFromSnapshotOnce(index));
    if (data)
        internal::PerformCastCheck(data);
...
template <class T>
size_t
SnapshotCreator::AddData(Local<Context> context, Local<T> object)
{
    T* object_ptr = *object;
...
template <class T>
size_t
SnapshotCreator::AddData(Local<T> object)
{
    T* object_ptr = *object;
    internal::Address* p = reinterpret_cast<internal::Address*>(object_ptr);
    return AddData(*p);
